...
const helpCommand = {
    build() {
        return new SlashCommandBuilder()
            .setName("help")
            .setDescription("Shows detailed information about the bot or specific commands.")
            .addStringOption((option) => option
            .setName("page")
            .setDescription("Select a specific category. Select all to see all commands and descriptions.")
            .setRequired(true)
        ...
        for (const file of readAllFiles(path.join("./dist/commands"))) {
            ...
            try {
                ...
                if (typeof commandModule.build === "function") {
                    ...
                    if (commandData.name) {
                        const description = commandData.description ?? "Context menu command."; // Default description for context
                        commandDetails.push({ name: commandData.name, value: description });
                    }
                }
                else {
                    console.warn(`The command at '${file}' does not conform to the expected structure.`);
                }
            }
            catch (error) {
