...
*/
export function missingPerms(memberPerms, requiredPerms) {
...
;
export function formatMessageToEmbed(message) {
...
export async function uploadDistToRemote() {
    RemoteClient.connect({
        ...
        await RemoteClient.uploadDir(srcToSrc, "/src").then(async () => {
            console.log("Uploaded".green.bold, "/src/".blue.bold, "to remote server!".green.bold);
            await RemoteClient.uploadDir(srcToDist, "/dist").then(() => {
                console.log("Uploaded".green.bold, "/dist/".blue.bold, "to remote server!".green.bold);
            });
        });
    });
}
async function removeSrcOnRemote(path) {
    await RemoteClient.rmdir(path, true).then(() => {
        console.log("Deleted".red.bold, "/src/".blue.bold, "from remote server!".red.bold);
    });
}
async function removeDistOnRemote(path) {
    await RemoteClient.rmdir(path, true).then(() => {
        console.log("Deleted".red.bold, "/dist/".blue.bold, "from remote server!".red.bold);
    });
}
